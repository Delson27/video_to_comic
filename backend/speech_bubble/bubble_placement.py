...
def get_bubble_position(crop_coord, CAM_data, is_normal_page=False):
    left, right, top, bottom = crop_coord
    x_ = CAM_data['x_']
    y_ = CAM_data['y_']
    ...
    if is_normal_page:
        # Force type '1' for normal pages (squares)
        panel_type = '1'
    else:
        # Use detected type for last page
        panel_type = get_panel_type(left, right, top, bottom)
    
    least_roi_x, least_roi_y = convert_to_css_pixel(least_roi_x, least_roi_y, crop_coord, is_normal_page)
